...
socket.on("chat message", function (data) {
  ...
  else var $usernameDiv = $('<span class="username"/>').text("Client");
  ...
  var $messageDiv = $('<li class="message"/>').append(
    $usernameDiv,
    $messageBodyDiv,
    $timestampDiv
  );
...
socket.on("New Client", function (data) {
  ...
  $chatHeader.append(
    data.details[0] + " , " + data.details[1] + " , " + data.details[2]
  );
  var sender;
  for (var i = len - 1; i >= 0; i--) {
    if (data["history"][i]["who"]) sender = "You";
    else sender = "Client";
    ...
    var $timestampDiv = $('<span class="timestamp">').text(
      data["history"][i]["when"].toLocaleString().substr(15, 6)
    );
    var $messageDiv = $('<li class="message"/>').append(
      $usernameDiv,
      $messageBodyDiv,
...
function getChatArea(id) {
  return (
    "<div class='chatArea'><div class='chatHeader'></div><ul class='messages'>" +
    "</ul><div class='typing'></div><input class='inputMessage' id='" +
    id +
