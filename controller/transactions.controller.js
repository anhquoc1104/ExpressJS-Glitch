...
module.exports = {
    home: async(req, res) => {
        try {
            ...
            for (let elm of arrTrans) {
                ...
                elm.time = `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()}`;
            }
            let obj = pagination.pagination(user, page, 10, 'books', arrTrans, '/transacions/page/');
            res.render("./transactions/transactions.pug", obj);
        } catch (err) {
            console.log(err);
    ...
    userTransaction: async(req, res) => {
        ...
        let obj = pagination.pagination(user, page, 10, 'books', arrTrans, `/transacions/${user._id}`);
    ...
    isComplete: async(req, res) => {
        ...
        if (!page) page = 1;
        let trans = await Transaction.findOneAndUpdate(id, { isComplete: "true" }, { new: true });
        // console.log(trans);
